...
test('web-client serves case list and 405/404 guards', async () => {
  ...
  try {
    ...
    const caseList = await fetch(`http://127.0.0.1:${port}/cases`)
    const caseListHtml = await caseList.text()
    assert.equal(caseList.status, 200)
    assert.match(caseListHtml, /Case list/)

    const methodNotAllowed = await fetch(`http://127.0.0.1:${port}/cases`, {
      method: 'POST',
    })
